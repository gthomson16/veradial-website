...
SHOTS_DIR="${BUILD_DIR}/shots-886"
BACK_DIR="${BUILD_DIR}/backplates-886"
...
# Canvas (App Preview iPhone 6.5"+ native dimensions)
CANVAS_W=886
CANVAS_H=1920

# Phone: smaller (0.62). Phone = 670x1190 at y=350..1540.
PHONE_SCALE=0.62
PHONE_TOP=350

# Text layer Y positions (captions at their approved "centered" positions).
BRAND_Y=55
EYEBROW_Y=125
TITLE_Y=185
SUB_Y=380
...
# --- Shot metadata table ------------------------------------------------------
# Each shot has 1..N ordered. Fields are sourced from website.md (see
# plan file /Users/gthomson/.claude/plans/bubbly-questing-spindle.md).
# Bullets and subtitles are newline-separated inside a single string.
...
SHOT_EYEBROW=("STEP ONE" "STEP TWO" "STEP THREE" "STEP FOUR")
...
SHOT_SUB2=("" "" "" "")
# Bullets: one per line, max 3 bullets per shot.
SHOT_BULLETS=(
...
# Shot durations + motion parameters (matches xfade schedule)
...
# --- Backplate: bg gradient + teal glow + drop shadow -------------------------
render_backplate_bg() {
  local out_png="$1"
  CANVAS_W="${CANVAS_W}" CANVAS_H="${CANVAS_H}" \
  PHONE_SCALE="${PHONE_SCALE}" PHONE_TOP="${PHONE_TOP}" \
  python3 - "$out_png" <<'PY'
import os, sys, math
...
# Radial gradient, lit from (W/2, H*0.30)
cx, cy = W / 2, H * 0.30
...
# Phone geometry
phone_w = int(W * phone_scale)
phone_h = int(H * phone_scale)
phone_cx = W // 2
phone_cy = phone_top + phone_h // 2

# Decorative concentric rings behind phone (frame_25 reference)
...
for radius, alpha in [(600, 22), (440, 28), (300, 28)]:
    gdraw.ellipse(
        (phone_cx - radius, phone_cy - radius, phone_cx + radius, phone_cy + radius),
        fill=(*ACCENT, alpha),
    )
...
# --- Layer: brand row (logo + wordmark + brand_suffix) -----------------------
render_brand_layer() {
  local out_png="$1"
  local brand_suffix="$2"

  CANVAS_W="${CANVAS_W}" CANVAS_H="${CANVAS_H}" BRAND_Y="${BRAND_Y}" \
...
img.alpha_composite(logo, (70, brand_y))
...
# Accent filled-circle separator between wordmark and suffix (matches reference)
dot_cx = dial_end + 18
dot_cy = wordmark_y + brand_size // 2
draw.ellipse((dot_cx - 3, dot_cy - 3, dot_cx + 3, dot_cy + 3), fill=ACCENT)

# Suffix: small gray, tracked uppercase (e.g. "01 · YOUR NUMBERS")
...
# --- Layer: eyebrow (disabled — empty transparent PNG) -----------------------
# Was "STEP ONE / STEP TWO..." pill with underline. Disabled per user request.
...
# --- Layer: title line 1 (white) ---------------------------------------------
render_title1_layer() {
  local out_png="$1"
  local text="$2"
  _render_title_line "$out_png" "$text" "white" 0
}

# --- Layer: title line 2 (accent) --------------------------------------------
render_title2_layer() {
  local out_png="$1"
  local text="$2"
  _render_title_line "$out_png" "$text" "accent" 1
}

# Shared: render a single title line at the correct y offset
_render_title_line() {
  local out_png="$1"
  local text="$2"
  local color_name="$3"
  local line_index="$4"   # 0 or 1

  CANVAS_W="${CANVAS_W}" CANVAS_H="${CANVAS_H}" TITLE_Y="${TITLE_Y}" \
  FONT_BOLD="${FONT_BOLD}" \
...
# Auto-size title: try 96..64 by 2 until fits within safe width
max_w = W - 140
# Cap at 84pt to leave room for eyebrow above and subtitle below (y=430)
for size in range(84, 60, -2):
...
# --- Layer: subtitle (2-line LABEL_GRAY supporting paragraph) ----------------
render_subtitle_layer() {
  local out_png="$1"
  local line1="$2"
  local line2="$3"
  CANVAS_W="${CANVAS_W}" CANVAS_H="${CANVAS_H}" SUB_Y="${SUB_Y}" \
...
for i, line in enumerate((line1, line2)):
    if line:
        draw.text((70, y0 + i * line_h), line, font=font, fill=LABEL_GRAY)

img.save(out_path)
PY
}

# --- Layer: bullets (small, accent-dot markers) ------------------------------
render_bullets_layer() {
  local out_png="$1"
  local bullets="$2"   # newline-separated list of bullet strings
  CANVAS_W="${CANVAS_W}" CANVAS_H="${CANVAS_H}" BULLETS_Y="${BULLETS_Y}" \
  FONT_SEMIBOLD="${FONT_SEMIBOLD}" \
...
for i, b in enumerate(bullets):
    cy = y0 + i * line_h + 18  # dot vertical center
    # Accent filled circle
...
# --- Layer: bottom micro-copy strip (centered one-liner) ---------------------
render_strip_layer() {
  local out_png="$1"
  local text="$2"
  CANVAS_W="${CANVAS_W}" CANVAS_H="${CANVAS_H}" STRIP_Y="${STRIP_Y}" \
  FONT_SEMIBOLD="${FONT_SEMIBOLD}" \
...
tw = tb[2] - tb[0]
draw.text(((W - tw) / 2, y0), text, font=font, fill=STRIP_GRAY)

img.save(out_path)
PY
}

# --- Phone body backing (solid black rounded-rect under the bezel) -----------
# The iPhone bezel PNG has a semi-transparent strip in the status-bar zone
# (native y=292-402). Without a black backing the backplate gradient shows
# through, making the phone top look translucent. This fills that strip.
render_phone_backing() {
  local out_png="$1"
  python3 - "$out_png" <<'PY'
import sys
from PIL import Image, ImageDraw
W, H = 1080, 1920  # matches bezel native resolution
img = Image.new("RGBA", (W, H), (0, 0, 0, 0))
draw = ImageDraw.Draw(img)
# Phone body in iphone-frame.png occupies x=165..915, y≈335..1920
# (there's transparent space above the phone in the PNG).
# Corner radius ~92px native. Bottom corners are at canvas edge so rounding
# is clipped — that's fine, the bezel's own silhouette draws the rounding.
...
for r, a in [(520, 50), (360, 60), (220, 60)]:
...
logo_y = 740
img.alpha_composite(logo, ((W - logo.width)//2, logo_y))
...
# --- Shot renderer ------------------------------------------------------------
# Inputs:
#   0=bg.png, 1=base app screenshot, 2=bezel.png,
#   3=brand, 4=eyebrow, 5=title1, 6=title2, 7=subtitle, 8=bullets, 9=strip
render_shot() {
  local out_file="$1"
  local base_png="$2"
  local bg_png="$3"
  ...
  # Bezel is a 1080x1920 image; preserve its aspect ratio independent of canvas width
  # so the phone doesn't get squished when canvas != 1080 wide.
  phone_h="$(awk "BEGIN { printf \"%d\", ${CANVAS_H} * ${PHONE_SCALE} }")"
  phone_w="$(awk "BEGIN { printf \"%d\", ${phone_h} * 1080 / 1920 }")"
  phone_x="$(awk "BEGIN { printf \"%d\", (${CANVAS_W} - ${phone_w}) / 2 }")"
  phone_y="${PHONE_TOP}"

  local app_w app_h app_x app_y
  app_w="$(awk "BEGIN { printf \"%d\", 720 * ${PHONE_SCALE} }")"
  # Bezel screen is 720x1482 native (at y=402). Crop scaled app to match height
  # so pans/zooms can't spill below the phone's screen area.
  ...
  # Staggered cascade reveal (Pass 7). Each layer fades in 0.3s.
...
render_wordmark_shot() {
  local out_file="$1"
  local duration="$2"
  local card_png="${SHOTS_DIR}/05-wordmark.png"
...
# --- Run: bg + per-shot layers + shot renders --------------------------------
...
for i in 0 1 2 3; do
  ...
  render_subtitle_layer "${BACK_DIR}/${n}-sub.png"      "${SHOT_SUB1[$i]}" "${SHOT_SUB2[$i]}"
...
# --- Concat with cross-dissolves ---------------------------------------------
echo "→ Concatenating"
SILENT_MASTER="${BUILD_DIR}/veradial-preview-886-silent.mp4"
# Chain with intro (1s) first, then shots, then end card. Each xfade is 0.25s.
# Offsets: intro→01 @0.75, 01→02 @4.50, 02→03 @9.25, 03→04 @17.00, 04→05 @20.75
# Final duration: 21.75s
ffmpeg -y \
  -i "${SHOTS_DIR}/00.mp4" \
  -i "${SHOTS_DIR}/01.mp4" \
  -i "${SHOTS_DIR}/02.mp4" \
...
# --- Audio mux: music version (shipping artifact) ----------------------------
MASTER="${BUILD_DIR}/veradial-preview-886-master.mp4"
if [[ -f "${MUSIC_FILE}" ]]; then
  echo "→ Muxing music into master"
  # Total video duration ≈ 21s (after xfades). Slice Glass_Interface from t=29
  # (best region of the track), fade in 1s / fade out 2s, loudnorm.
  ...
  echo "⚠ ${MUSIC_FILE} not found — skipping audio mux. Master = silent."
  cp "${SILENT_MASTER}" "${MASTER}"
fi

echo
echo "Silent master: ${SILENT_MASTER}"
echo "Master (audio): ${MASTER}"
