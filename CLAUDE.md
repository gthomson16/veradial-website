...
- **GSC / Indexing / GA4 CLI:** scripts under `~/.claude/skills/seo/scripts/`: `gsc_query.py` (search analytics, sitemaps), `gsc_inspect.py` (URL indexation status, `--batch` supported), `indexing_notify.py` (Web Indexing API pings, 200/day quota), `ga4_report.py` (GA4 organic traffic), `pagespeed_check.py` (PSI lab + CrUX). GCP project: `canscanapp`; GA4 property: `properties/532843651`; GSC property: `sc-domain:veradial.com`. Preferred auth is keyless service-account impersonation via `veradial-seo-audit@canscanapp.iam.gserviceaccount.com` configured in `~/.config/claude-seo/google-api.json`; org policy blocks long-lived service-account JSON keys, so keep using impersonation. The service account still needs product-level access before GA4/GSC reports will return data: GA4 Property Access Management as Viewer, and Search Console as a delegated owner from the ownership verification / verified-owner flow. Do not use Search Console's regular "Add user" / Full-user path for this service account; that path requires a normal Google Account and can reject `iam.gserviceaccount.com` addresses.
